...
export default {
  ...
  methods: {
    ...
    async setPassword() {
      //设置密码
      let { Code, Message } = await this.$store.dispatch("SetPassword", {
    ...
    async registered() {
      ...
      this.$store.dispatch("Registered", formData).then(
        response => {
          Toast({
            message: "注册成功",
            position: "bottom"
    ...
    async setPasswordNext() {
      //忘记密码
      let { Code, Message, Data } = await this.$store.dispatch("GetUserInfo", {
    ...
    async registeredSendPhoneMessage() {
      //获取验证码
